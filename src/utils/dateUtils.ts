 ...
 */
export function formatMonth(date: Date, locale = "pt-BR"): string {
...
export function getDuration(
	...
	try {
		...
		switch (viewMode) {
			...
			case ViewMode.WEEK:
				const daysInWeek = differenceInDays(laterDate, earlierDate) + 1;
				if (daysInWeek < 7) {
					return { value: daysInWeek, unit: daysInWeek === 1 ? "day" : "days" };
				}
...
export function calculateDuration(startDate: Date, endDate: Date): number {
	...
	console.log("[calculateDuration] diffTime:", diffTime);
	const duration = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
	console.log("[calculateDuration] duration (days):", duration);
	return duration;
}
