...
const GanttChart: React.FC<GanttChartProps> = ({
	...
	let derivedEndDate = customEndDate || findLatestDate(tasks);

	// Ensure timeline includes currentDate
	if (currentDate > derivedEndDate) {
		const newEnd = new Date(currentDate);
		derivedEndDate = new Date(newEnd.getFullYear(), newEnd.getMonth() + 1, 0);
	}
