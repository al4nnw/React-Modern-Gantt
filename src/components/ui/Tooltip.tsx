...
> = ({
	...
	try {
		...
		// Only calculate live dates if we are actively dragging/resizing
		// Otherwise, rely on the actual task data to avoid rounding errors from pixel conversion
		if (dragType && taskEl) {
	...
	const formatDate = (date: Date) => {
		...
		switch (viewMode) {
			case ViewMode.MINUTE:
				return format(date, "MMM d, yyyy HH:mm", { locale: getLocale() });
			case ViewMode.HOUR:
				return format(date, "MMM d, yyyy HH:00", { locale: getLocale() });
			case ViewMode.DAY:
				return format(date, "EEE, MMM d, yyyy", { locale: getLocale() });
			default:
		...
		>
			...
			<div className="rmg-tooltip-content" data-rmg-component="tooltip-content">
				<div className="rmg-tooltip-row" data-rmg-component="tooltip-row">
					<div className="rmg-tooltip-label">Início:</div>
				...
				<div className="rmg-tooltip-row" data-rmg-component="tooltip-row">
					<div className="rmg-tooltip-label">Término:</div>
					<div className="rmg-tooltip-value">{formatDate(displayEndDate)}</div>
				</div>

				<div className="rmg-tooltip-row" data-rmg-component="tooltip-row">
					<div className="rmg-tooltip-label">Duração:</div>
					<div className="rmg-tooltip-value">
				...
				{task.dependencies && task.dependencies.length > 0 && (
					<div className="rmg-tooltip-row" data-rmg-component="tooltip-row">
						<div className="rmg-tooltip-label">Dependências:</div>
						<div className="rmg-tooltip-value">
